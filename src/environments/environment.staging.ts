export const environment = {
  production: true,
  dashboardApiUrl: 'https://cwpdemo.innovapptive.com/dashboardapi/',
  mccAbapApiUrl: 'https://cwpdemo.innovapptive.com/mccspccabapapi/',
  spccAbapApiUrl: 'https://cwpdemo.innovapptive.com/mccspccabapapi/',
  wiApiUrl: 'https://cwpdemo.innovapptive.com/wiapi/',
  wiAbapApiUrl: 'https://cwpdemo.innovapptive.com/wiabapapi/',
  userRoleManagementApiUrl:
    'https://cwpdemo.innovapptive.com/userrolemanagementapi/',
  rdfApiUrl: 'https://cwpdemo.innovapptive.com/rdfapi/',
  masterConfigApiUrl: 'https://cwpdemo.innovapptive.com/masterconfigapi/',
  operatorRoundsApiUrl: 'https://cwpdemo.innovapptive.com/operatorroundsapi/',
  s3BaseUrl: 'https://innovwi.s3.ap-south-1.amazonaws.com/',
  requestTimeout: 60000 * 3,
  undoRedoOffset: 5,
  jaasAppID: 'vpaas-magic-cookie-c9a785fe985444a18ba0c24416de0d6c'
};
