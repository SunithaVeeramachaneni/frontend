...
export class StepContentComponent implements OnInit, OnDestroy {
  ...
  deleteStep (el) {
    Swal.fire({
      title: 'Are you sure?',
      html: "Selected step <strong>" + "'" + el.Title + "'" + "</strong> gets deleted ",
      icon: 'warning',
    ...
    this._instructionSvc.removeStep(step).subscribe((resp) => {
      if (Object.keys(resp).length) {
        this._toastService.show({
          text: "Step is deleted successfully!",
          type: 'success',
        });
        this._commonSvc.stepDetailsSave('All Changes Saved');
  ...
  deleteAtt(file, index) {
    Swal.fire({
      title: 'Are you sure?',
      html: "Attachment <strong>" + "'" + file + "'" + "</strong> gets deleted ",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (Object.keys(step_resp).length) {
        ...
        this._instructionSvc.updateStep(step_resp).subscribe((step) => {
          if (Object.keys(step).length) {
            ...
            this._toastService.show({
              text: "Attachment is deleted successfully!",
              type: 'success',
            });
            this.editByUser();
            this._commonSvc.stepDetailsSave('All Changes Saved');
            ...
            if (stepResp &&  Object.keys(stepResp).length > 0) {
              ...
              this._instructionSvc.addStep(step).subscribe((resp) => {
                if (Object.keys(step).length) {
                  ...
                  this._commonSvc.setUpdatedSteps(this.steps);
                  this._toastService.show({
                    text: "Step Title has been added!",
                    type: 'success',
                  });
                  document.getElementById("step_title").blur();
