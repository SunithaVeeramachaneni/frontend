...
const categoryDetails = [
  ...
  },
  {
    ...
    Cover_Image:
      'assets/work-instructions-icons/work-instructions-icons/CoverImages/coverimage2.png'
  },
  {
    Category_Id: 178,
...
const TOOLS =
  '[{"Title":"Tools","Position":0,"Active":"true","FieldCategory":"HEADER","FieldType":"RTF","FieldValue":["test"]},{"Title":"SafetyKit","Position":1,"Active":"true","FieldCategory":"HEADER","FieldType":"RTF","FieldValue":["test"]},{"Title":"SpareParts","Position":2,"Active":"true","FieldCategory":"HEADER","FieldType":"RTF","FieldValue":["test"]}]';
const ASSIGNEDOBJECTS =
  '[{"OBJECTCATEGORY":"WORKORDER","FILEDNAME":"ATNAM","FIELDDESCRIPTION":"CHARACTERISTIC NAME","Value":"123"}]';
...
describe('AddWorkinstructionComponent', () => {
  ...
  beforeEach(() => {
    ...
    mockInsToBePublishedSelector = store.overrideSelector(
      getInsToBePublished,
      []
    );
  ...
  describe('ngOnInit', () => {
    ...
    it('should set breadcrumb & header title', () => {
      ...
      ).and.returnValue(of(`work-instructions/edit/${Id}`));
      ...
      fixture.whenStable().then(() => {
        ...
        expect(store.dispatch).toHaveBeenCalledOnceWith(
          InstructionActions.updateInstruction({
            instruction: { ...editWI, Published: true }
          })
        );
        expect(component.selectedInstruction).toEqual({
          ...editWI,
          Published: true
        });
        expect(component.titleProvided).toBeTrue();
    ...
    it('should handle title text change for add work instruction', fakeAsync(() => {
      ...
      tick(1001);
      expect(instructionServiceSpy.getInstructionsByName).toHaveBeenCalledWith(
        WIName,
        info
      );
  ...
  describe('publishInstruction', () => {
    ...
    it('should publish already publsihed instruction with edited details when user click on publish', (done) => {
      ...
      (instructionServiceSpy.publishInstruction as jasmine.Spy)
        ...
        .and.returnValue(
          of([{ ...editWI, EditedBy: 'Tester One', Published: true }])
        )
        .and.callThrough();
      mockInsToBePublishedSelector.setResult([
        { ...insToBePublished[0], PUBLISHED: true }
      ]);
      ...
      fixture.whenStable().then(() => {
        expect(store.dispatch).toHaveBeenCalledWith(
          InstructionActions.setInsToBePublished()
        );
        expect(component.insToBePublished).toEqual([
          { ...insToBePublished[0], PUBLISHED: true }
        ]);
    ...
    it('should publish new work instruction when user click on publish', (done) => {
      ...
      (instructionServiceSpy.publishInstruction as jasmine.Spy)
        .withArgs(
          {
            wiToBePublsihed: [
              { ...insToBePublished[0], FORMTITLE: addWI[0].WI_Name }
            ],
            steps: [],
            wid: addWI[0].Id,
            editedBy: 'Tester One'
      ...
      fixture.whenStable().then(() => {
        ...
        expect(component.updateFavFlag).toHaveBeenCalledWith([
          { ...insToBePublished[0], FORMTITLE: addWI[0].WI_Name }
        ]);
        expect(component.publisheddata).toBeTrue();
        expect(component.receivedInstruction).toBeFalse();
    ...
    it('should handle error while publishing work instruction', (done) => {
      ...
      fixture.whenStable().then(() => {
        ...
        expect(errorHandlerServiceSpy.handleError).toHaveBeenCalledWith({
          message: 'Unable to publish work instruction'
        } as HttpErrorResponse);
        expect(spinnerSpy.hide).toHaveBeenCalledWith();
        done();
      });
  ...
  describe('updateFavFlag', () => {
    ...
    it('should update fav flag for newly publishing work instruction if it is in favorite state', () => {
      ...
      expect(
        instructionServiceSpy.updateGatewayFavWorkInstruction
      ).toHaveBeenCalledWith(favInstructionData);
    });
  });
  ...
  describe('getWorkInstruction', () => {
    ...
    it('should set publish related flags, selectedInstruction and call draftWI on WI header details entry', () => {
      ...
      overviewComponent.instructionDataEntry.emit({
        insObj: editWI,
        update: true
      });
  ...
  describe('updatePublishedTillSaveWI', () => {
    ...
    it('should update published till save flag to 0 if work instruction is not published till saved', () => {
      ...
      expect(store.dispatch).toHaveBeenCalledWith(
        InstructionActions.updateInstruction({ instruction: payload })
      );
    });
    ...
    it('should not update published till save flags if work instruction is not published', () => {
      ...
      expect(
        instructionServiceSpy.updateWorkInstruction
  ...
  describe('copyWI', () => {
    ...
    it('should copy work instruction while clicking on copy work instruction from mat menu', () => {
      (instructionServiceSpy.copyWorkInstruction as jasmine.Spy)
        ...
        .and.returnValue(
          of({
            instruction: { ...editWI, WI_Name: 'Name of Copy Inst' },
            steps: []
          })
        );
  ...
  describe('removeWI', () => {
    ...
    it('should remove work instruction when click on confirm/delete', (done) => {
      ...
      setTimeout(() => {
        ...
        ).toHaveBeenCalledWith(editWI.Id, info);
        expect(
          instructionServiceSpy.deleteWorkInstruction$
  ...
  describe('addTitleToInstruction', () => {
    ...
    it('should edit existing work instruction title', () => {
      ...
      expect(
        ...
        editId,
        loggedInUser,
        selectedInstructionNew,
        info
      );
      expect(store.dispatch).toHaveBeenCalledWith(
        InstructionActions.updateInstruction({
          instruction: selectedInstructionNew
        })
      );
      expect(wiCommonServiceSpy.stepDetailsSave).toHaveBeenCalledWith(
        'All Changes Saved'
    ...
    it('should handle error while editing existing work instruction title', () => {
      ...
      (instructionServiceSpy.editWorkInstructionTitle as jasmine.Spy)
        .withArgs(editId, loggedInUser, selectedInstructionNew, info)
        .and.returnValue(
          throwError({ message: 'Unable to update work instruction' })
        )
        .and.callThrough();
      component.ngOnInit();
      fixture.detectChanges();
      ...
      expect(errorHandlerServiceSpy.handleError).toHaveBeenCalledWith({
        message: 'Unable to update work instruction'
      } as HttpErrorResponse);
    });
  });
  ...
  describe('ngOnDestroy', () => {
    ...
    it('should unsubscribe subscriptions', () => {
      ...
      expect(store.dispatch).toHaveBeenCalledWith(
        InstructionActions.resetInstructionState()
      );
      expect(component.insToBePublished).toEqual([]);
      expect(
      ...
      expect(component['stepsSubscription'].unsubscribe).toHaveBeenCalledWith();
    });
