...
describe('CategoriesComponent', () => {
  ...
  describe('template', () => {
    ...
    it('should render three categories in UI', () => {
      ...
      expect(categoriesEl.querySelectorAll('pagination-template').length).toBe(
        1
      );
      expect(
        categoriesEl.querySelectorAll('app-custom-pagination-controls').length
      ).toBe(1);
    });
  });
  ...
  describe('getAllCategories', () => {
    ...
    it('should set categoriesList', () => {
      ...
      ).toHaveBeenCalledWith(defaultCategoryId);
      expect(
        instructionServiceSpy.getInstructionsByCategoryId
      ).toHaveBeenCalledWith('177');
      expect(
        instructionServiceSpy.getInstructionsByCategoryId
    ...
    it('should call getBase64Image if Cover_Image is not from assets', () => {
      ...
      (instructionServiceSpy.getAllCategories as jasmine.Spy)
        ...
        .and.returnValue(
          of([
            categoryDetails[0],
            { ...categoryDetails[1], Cover_Image: 'Thumbnail.jpg' }
          ])
        )
        .and.callThrough();
      (instructionServiceSpy.getInstructionsByCategoryId as jasmine.Spy)
  ...
  describe('open', () => {
    ...
    it('should add new Category, if cover image is from S3 rename file to cover image id folder', () => {
      ...
      (instructionServiceSpy.renameFile as jasmine.Spy)
        .withArgs({
          filePath: `${path}/cover-image-from-s3.png`,
          newFilePath: `${response.Category_Id}/cover-image-from-s3.png`
        })
        .and.returnValue(
          of({
            filePath: `${path}/cover-image-from-s3.png`,
    ...
    it('should delete files from category service & s3 bucket in case of uploaded file to S3 and add category modal closed without saving', () => {
      ...
      expect(instructionServiceSpy.deleteFile).toHaveBeenCalledWith(
        `${path}/${files[0]}`
      );
  ...
  describe('getS3CoverImageStyles', () => {
    ...
    it('should return empty styles if cover image is from assets', () => {
      const result = component.getS3CoverImageStyles(
        'assets/work-instructions-icons/coverimage.jpg'
      );
      expect(result).toEqual({});
    });
  });
  ...
  describe('ngOnDestroy', () => {
    ...
    it('should unsubscribe subscription', () => {
      ...
      expect(
        component['fetchCategoriesSubscription'].unsubscribe
