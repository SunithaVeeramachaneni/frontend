...
fdescribe('User service', () => {
  ...
  beforeEach(() => {
    ...
    (appServiceSpy._getResp as jasmine.Spy)
      ...
      .and.callThrough();
    (appServiceSpy._getResp as jasmine.Spy)
      .withArgs(environment.usersAndPermissionsUrl, 'roles', {
  ...
  it('needs to get active users count', () => {
    service.getUsersCount$({ isActive: true }).subscribe((resp) => {
      expect(resp).toEqual({ count: 3 });
    });
  });

  it('needs to get all roles', () => {
    const allRoles = service.getRoles$();
    allRoles.subscribe((res) => {
      expect(res).toEqual(allRolesMock);
    });
  });
  ...
  it('needs to get user from the backend and prepare it for the frontende', () => {
    const preparedUser = service.prepareUser(usersMock[0], rolesByID1Mock);

    expect(
      isEqual(JSON.stringify(preparedUser), JSON.stringify(preparedUserMock))
    ).toBeTrue();
  });
