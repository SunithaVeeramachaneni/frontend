...
export class HttpRequestInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (request.headers.get('authorization') === null) {
      const protectedResource = this.isProtectedResource(request.url) || [];

      if (protectedResource.length) {
        const [urls, scope] = protectedResource;
        const { token_type, access_token, access_token_expires_at } = JSON.parse(sessionStorage.getItem(hash(urls))) || {};

        if (access_token && new Date().getTime() < access_token_expires_at - 30000) {
          const cloneRequest = request.clone({
            headers: request.headers.set('authorization', `${token_type} ${access_token}`)
          });
  ...
  isProtectedResource(requestUrl: string) {
    return this.commonService.getProtectedResources().find(protectedResource => {
