...
describe('BulkUploadComponent', () => {
  ...
  describe('close', () => {
    ...
    it('should navigate to successUrl when click on okay if upload is success and drafted instruction count is non zero', () => {
      ...
      expect(router.navigate).toHaveBeenCalledWith(['/work-instructions/drafts']);
    });
  ...
  describe('deleteIns', () => {
    ...
    it('should delete instruction without alert', () => {
      ...
      const ins = [
        {instructionName: 'Sample WorkInstruction1', insPostedSuccessfully: true, insPostingFailed: true, id: Id, WI_Id},
        {instructionName: 'Sample WorkInstruction3', insPostedSuccessfully: true, insPostingFailed: false, id: id1, WI_Id: WI_Id1}
      ];
    ...
    it('should delete instruction with alert', () => {
      ...
      const { Id: id1, WI_Id: WI_Id1} = inst3Resp;
      const ins = [
        {instructionName: 'Sample WorkInstruction1', insPostedSuccessfully: true, insPostingFailed: false, id: Id, WI_Id},
        {instructionName: 'Sample WorkInstruction3', insPostedSuccessfully: true, insPostingFailed: false, id: id1, WI_Id: WI_Id1}
      ];
      (instructionServiceSpy.deleteWorkInstruction$ as jasmine.Spy)
        .withArgs(ins[0].id, info)
      ...
      expect(alertServiceSpy.success).toHaveBeenCalledWith('Instruction has be deleted');
      expect(component.ins).toEqual([{ ...ins[0], insDeletedSuccessfully: true }, ins[1]]);
    });
    ...
    it('should handle error', () => {
      ...
      (instructionServiceSpy.deleteWorkInstruction$ as jasmine.Spy)
        .withArgs(ins[0].id, info)
        .and.returnValue(throwError({ message: 'Unable to delete instruction'}));
      component.ins = ins;

      component.deleteIns(ins[0], 0, false);

      expect(errorHandlerServiceSpy.getErrorMessage).toHaveBeenCalledWith({ message: 'Unable to delete instruction'} as HttpErrorResponse);
      expect(alertServiceSpy.error).toHaveBeenCalledWith(errorHandlerServiceSpy.getErrorMessage({ message: 'Unable to delete instruction'} as HttpErrorResponse));
    });
    ...
    it('should delete instruction when click on delete icon', () => {
      ...
      const buttons = bulkUploadEl.querySelectorAll('.bulk-upload-instructions-list .pull-right mat-icon');
