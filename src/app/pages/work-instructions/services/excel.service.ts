...
export class ExcelService {
  ...
  public exportAsExcelFile(book, sheets, excelFileName: string): void {
    ...
    const cols = [
      ...
      // tslint:disable-next-line:max-line-length
      { header: 'AssignedObjects', key: 'assignedObjects', width: 64.11, height: 35, style: { font: { name: 'Calibri', size: 12,  bold: true } } },
    ...
    for (let sheetCnt = 0; sheetCnt < sheets.length; sheetCnt++) {
      ...
      sheets[sheetCnt].addRow({ workInstruction: 'Sample WorkInstruction' + (sheetCnt + 1), assignedObjects: 'Sample Label1: Sample Value1, Sample Label2: Sample Value2', category: 'Sample Category' + (sheetCnt + 1), safetyKit: 'Sample Kit1, Sample Kit2 ', spareParts: 'Sample Spare Part1, Sample Spare Part2', tools: 'Sample Tool1, Sample Tool2',  cover_image_name: 'doc-placeholder.png', cover_image: '', stepTitle: '', instruction: '', warning: '', reactionPlan: '', hint: '', attachment_1_name: '', attachment_1: '', attachment_2_name: '', attachment_2: '' });
      ...
      sheets[sheetCnt].getCell('B1').note = 'Please enter exactly same name for the assigned object as per listed in the application. We can find the list in the Assigned Objects dropdown under the Header section\'.';
      ...
      sheets[sheetCnt].getCell('L1').note = 'Add Reaction Plan as numbered or bulleted list or as a paragraph in each step. By default, two sample reaction plans are added in this template. Add any number of Reaction Plans as required.';
