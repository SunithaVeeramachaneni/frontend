...
describe('RecentsComponent', () => {
  ...
  describe('template', () => {
    ...
    it('should contain labels related to recents', () => {
      const header = recentsEl.querySelector('.recents-table-title h2');
      expect(header.textContent).toContain('Recents');
      ...
      expect((anchors[0] as HTMLElement).getAttribute('href')).toBe(
        `/recents/add-instruction/${recent1.Id}`
      );
      ...
      // ).toBe(`/recents/add-instruction/,${recent1.Id}`);
      ).toContain(`/recents/add-instruction/`);
      ...
      expect(recentsEl.querySelectorAll('ngx-spinner').length).toBe(1);
    });
