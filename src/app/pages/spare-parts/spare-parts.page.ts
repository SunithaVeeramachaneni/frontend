...
export class SparePartsComponent{
 ...
 public imageUrl;
  ...
  getTechnicians(){
    this.technicians$ =this._sparepartsSvc.getPickerList();
  }
  ...
  getWorkOrders() {
    ...
    this.filteredWorkOrderList$ = combineLatest([this.workOrderList$,this.filterObj$]).pipe(
      map(([workOrders,filterObj]) => {
        let filtered: WorkOrders = {"1":[], "2":[], "3":[], "4":[],"5":[]};
