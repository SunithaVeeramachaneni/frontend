...
const getSelectOptions = () => {
    const selectOptionsList = ['PRIOK', 'PRIOKX', 'COLOUR', 'AUFNR', 'AUFTEXT', 'ARBPL', 'KTEXT', 'PARNR','IPHAS', 'WorkOrderOperationSet/STATUS','WorkOrderOperationSet/ARBEI', 'IPHAS', 'GSTRP']
...
app.get('/abapapi/workOrdersAndOperations', async (req, res) => {
    console.time("constructing options")
    let url = baseUrlWorkOrder + `WorkOrdersCollection?$expand=WorkOrderOperationSet&` + getSelectOptions() + '$format=json&';
    let isLastPage = false;
    let isPaginated = true//!!req.query.pagination;
    console.timeEnd("constructing options")
    console.log("Pagination", isPaginated)
    let workOrders = [];
